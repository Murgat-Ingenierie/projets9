...
| R16 | **Les comptes importés ne se rapprocheront pas de Keycloak** | 🔴 **à traiter avant que les équipes se connectent** | L'import fabrique les adresses des chargés de projet (`slug_email()` : nom concaténé + `@lesfontaines.fr`, domaine choisi parce qu'`email-validator` refuse `.local`). Elles ne désignent aucune boîte réelle. Or le rapprochement d'un jeton se fait par `keycloak_sub` puis **à défaut par email** : à la première connexion, chacun se verra donc créer un **second** compte. Constaté en base le 2026-07-29 — 4 comptes `@lesfontaines.fr` portant **51 tâches**, aucun lié au realm ; le seul compte lié est un `@gmail.com`, créé par une vraie connexion. **Deux conséquences** : le travail reste sur le compte fantôme (filtres par responsable vides pour l'intéressé), et chaque doublon consomme une place — 6 comptes + 4 doublons + admin franchit INV-AUTH-2, dont le refus s'affiche « Connexion impossible » sans mentionner la capacité. **Décision de Charles (2026-07-29) : ne PAS corriger les adresses ici**, il rapprochera les comptes directement en base. Ne pas « réparer » `slug_email` en croyant bien faire — le format est délibéré, c'est le rapprochement qui est manuel. Palliatif en place : `MAX_ACTIVE_USERS` est devenu configurable, de quoi absorber les doublons en attendant. |
