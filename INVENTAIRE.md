...
| | ↳ *Jalons et Dépendances sur mobile* | ✅ **le 2026-08-25** | Même patron que Projets/Tâches, avec une distinction : **Jalons se replie** (il a un nom), **Dépendances non** — une dépendance est une RELATION entre deux tâches, la réduire à l'une d'elles perdrait ce qu'elle dit. Recherche libre ajoutée aux deux : à cette largeur, les filtres par colonne vivent dans un en-tête masqué, elle est donc le seul moyen de filtrer. <br>**Régression de #106 corrigée au passage** : la règle de repliage visait TOUTE table `responsive` et masquait donc toutes les cellules de celles sans colonne-titre — la table des allocations d'une tâche était **invisible sur mobile** depuis, sans que rien ne le signale (vérifié : `display: none` sur ses quatre cellules). Le repliage est devenu un opt-in explicite (`repliable`). <br>Autre défaut trouvé : la police d'icônes venant de Google Fonts, tant qu'elle n'est pas arrivée la ligature ne s'applique pas et « expand_more » s'affiche littéralement — 130 px qui débordaient. L'icône est bornée. |
| | ↳ *atteindre une tâche et son journal* | ✅ **le 2026-08-25** | Deux retours d'usage, même sujet. Un bouton **« Activité »** par ligne ouvre la tâche sur son journal (`#activite`), qui vit en bas de page : « Ouvrir » y menait déjà, mais il fallait le savoir puis défiler. Le défilement se déclenche à l'ARRIVÉE des entrées, pas au montage — la section est alors à sa hauteur définitive — et une seule fois, sinon chaque écriture ramènerait l'écran ici. <br>Et les tâches listées sur la page d'un **projet** ont maintenant une issue : elles étaient affichées sans lien, on lisait un nom sans pouvoir l'ouvrir. Le fil d'Ariane reprend le projet comme parent, pour que le retour ramène là d'où l'on vient. <br>Effet de bord des tests : `JournalActivite` lit désormais le fragment d'URL, donc ses tests de composant ont besoin d'un `MemoryRouter`. |
| | ↳ *filtre responsable* | ✅ **le 2026-08-25** | Retour d'usage. La recherche libre trouvait déjà un nom, mais elle se tape de mémoire — une faute ne rend rien, et surtout elle ne dit pas QUI existe. Un sélecteur montre l'annuaire, avec **« Sans responsable »** : retrouver ce que personne ne porte est un besoin en soi. Sur Projets et Tâches, et il vaut d'autant plus sur écran étroit, où les filtres par colonne disparaissent avec l'en-tête. <br>Le filtre est appliqué **dans** `useSortableList` (paramètre `predicat`) et non par un `items.filter()` en amont : sinon `totalCount` compterait après filtrage, et « 1 sur 3 » deviendrait « 1 sur 1 ». Un test le verrouille. <br>Au passage, l'API simulée des e2e ne servait pas `/api/users/annuaire` — tout sélecteur de personne s'y affichait vide, et un test dessus n'aurait rien prouvé. |
| | ↳ *navigation mobile* | ✅ **le 2026-08-25** | Retour d'usage. Le rail de 64 px prenait **17 %** d'un écran de 375, en permanence, pour une navigation qu'on utilise quelques secondes : il devient un TIROIR hors champ, ouvert par un bouton flottant, refermé au voile ou après avoir suivi un lien. Le bouton d'ouverture est rendu À PART du tiroir — celui qu'il contient part avec lui hors du champ, et sert donc à refermer. <br>Le planning n'est plus **rendu** sous le seuil : `/` y redirige vers Projets (`replace`, sinon le retour arrière rebondit en boucle). Ni la navigation ni le fil d'Ariane ne le proposent — un lien qui renvoie systématiquement ailleurs déroute plus que son absence ; le fil est filtré dans `Breadcrumb`, pas dans chacune des pages. <br>`useEcranEtroit` duplique le seuil de 720 px que porte la feuille de style : CSS et JavaScript ne peuvent pas partager une valeur, les deux doivent bouger ensemble. |
...
| ~~R18~~ | ~~**La CI testait sur un Node que rien n'expédie**~~ | ✅ **résolu (2026-08-06)** | `web/Dockerfile` construit le bundle avec `node:26-alpine` ; les deux jobs Node de la CI étaient restés épinglés à `"20"`. On validait donc sur un moteur qu'aucun artefact n'utilise — le principe « on teste sur la version qu'on EXPÉDIE », déjà écrit pour Python 3.14, n'avait pas été appliqué à Node. **Révélé par une PR Dependabot** : jsdom 30 échouait sur `webidl.util.markAsUncloneable is not a function`, absente de Node 20 (jsdom 30 exige `^22.22.2 \|\| ^24.15.0 \|\| >=26`). Le paquet était sain, c'est le moteur de test qui était en retard sur l'image. CI alignée sur 26. |
