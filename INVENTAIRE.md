...
| | ↳ *redimensionnement des projets* | ⛔ **écarté (2026-08-04)** | Un projet portant des tâches est un `summary` pour SVAR, dont l'étendue dérive des enfants : ses barres n'ont aucune poignée, et `ISummaryConfig` n'expose rien pour l'activer (vérifié). **Décision : les dates d'un projet se règlent au panneau d'édition**, pas au geste. Ne pas rouvrir sans nouvel élément — la seule voie serait de sortir les projets du type `summary`, au prix du rendu groupé. |
| | ↳ *marque « terminé »* | ✅ **complété le 2026-08-04** | Une tâche archivée portait déjà hachure + coche ; **un projet `réalisé` n'affichait rien** — il fallait ouvrir la ligne pour le savoir. Le drapeau `archived` (nom devenu faux dès qu'il couvre deux entités) est renommé `termine` et posé aussi sur les projets. `abandonne` n'en reçoit pas : le planning ne doit pas affirmer un aboutissement qui n'a pas eu lieu. La coche tire désormais sa couleur du fond (`couleurTexteSur`, comme le libellé depuis #85) — son vert codé en dur passait sur les barres de tâches, éclaircies, mais devenait illisible sur la couleur pleine d'un epic. |
| | ↳ *dépendances masquées par le filtre* | ✅ **le 2026-08-04** | Sous filtre équipe, le planning ne trace un lien que si ses DEUX extrémités sont visibles — choix délibéré (un lien pendant ne veut rien dire, et une cascade déplacerait ce qu'on ne voit pas), mais dont personne n'avait mesuré le coût : une tâche dont les dépendances sortent du périmètre paraissait indépendante. Signalé par l'utilisateur test. Une marque (chaîne) apparaît désormais sur ces tâches, et son infobulle NOMME les liens cachés (« Dépend de « Pose et calibration » (FS) ») — connaître leur nombre sans pouvoir dire lesquels ne servirait à rien. Rien n'est signalé hors filtre, où les flèches sont toutes dessinées. Logique pure dans `web/src/planning/liensMasques.ts`. |
...
| R13 | Code mort | 🟡 | ~~INV-9/INV-13 (§5)~~ ✅ purgés en étape 0, ~~`Boolean` (epic)~~ ✅ retiré par `ruff --fix` — restent : `_slug()` (seed) et `equipes.get` (déclaré, jamais appelé) — vérifiés le 2026-08-04. ~~`nav`/`navState` (GanttPage)~~ : **plus mort**, `navState` sert dans `DependenciesPage` et `EpicDetailPage`. |
